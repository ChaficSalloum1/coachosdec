...
CREATE TABLE IF NOT EXISTS coaches (
  ...
  CONSTRAINT coaches_auth_user_unique UNIQUE (auth_user_id)
);
...
-- Coaches can only see or mutate their own records
CREATE POLICY "coaches_owner_full_access" ON coaches
  FOR ALL USING (auth_user_id = auth.uid()) WITH CHECK (auth_user_id = auth.uid());

-- Core resources scoped to the requesting coach
CREATE POLICY "areas_owner_full_access" ON areas
...
CREATE POLICY "facilities_owner_full_access" ON facilities
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "courts_owner_full_access" ON courts
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "students_owner_full_access" ON students
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "student_notes_owner_full_access" ON student_notes
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "lessons_owner_full_access" ON lessons
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

-- Booking requests allow public inserts (used by booking form) but restrict coach access
CREATE POLICY "booking_requests_public_insert" ON booking_requests
  FOR INSERT
  WITH CHECK (coach_id IN (SELECT id FROM coaches));

CREATE POLICY "booking_requests_owner_full_access" ON booking_requests
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "availability_ranges_owner_full_access" ON availability_ranges
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));

CREATE POLICY "blackout_dates_owner_full_access" ON blackout_dates
  FOR ALL USING (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()))
  WITH CHECK (coach_id IN (SELECT id FROM coaches WHERE auth_user_id = auth.uid()));
